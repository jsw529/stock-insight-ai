...
if st.button("분석하기") and ticker:
    try:
        ...
        is_us_stock = currency == "USD"
        rate = get_exchange_rate() if is_us_stock else 1
        ...
        if df is None or df.empty or 'Close' not in df:
            ...
            high_price = df["High"].max() * rate
            low_price = df["Low"].min() * rate

            st.subheader("💰 현재 주가 정보")
            st.metric(label="현재가", value=f"{current_price:,.0f} 원")
            st.metric(label="3개월 최고가", value=f"{high_price:,.0f} 원")
            st.metric(label="3개월 최저가", value=f"{low_price:,.0f} 원")

            st.subheader("📈 주가 차트 (최근 3개월)")
            fig, ax = plt.subplots()
            ax.plot(df["Date"], df["Close"] * rate, label="종가", color='blue')
            ax.set_xlabel("날짜")
            ax.set_ylabel("가격 (원화 환산)")
            ax.legend()
            st.pyplot(fig)
